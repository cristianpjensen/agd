...
class MLPGateCFGAdapter(Adapter):
    ...
    def __init__(
        ...
        self.mlp = MLP(2 * hidden_dim, hidden_dim, 1.0)

        nn.init.xavier_uniform_(self.gate[0].weight, gain=1.0)
    ...
    def _adapter_forward(
        ...
        x = torch.cat([x, c.unsqueeze(-2).expand_as(x)], dim=-1)  # (...B, T, 2 * hidden_dim)

        gates = self.gate(x)  # (...B, T, 1)
        feats = self.mlp(x)   # (...B, T, hidden_dim)
    
        return gates * self.out_proj(feats)
