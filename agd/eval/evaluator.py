"""Source: https://github.com/openai/guided-diffusion/blob/main/evaluations/evaluator.py."""
...
class Evaluator:
    ...
    def compute_activations(self, batches: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        preds = []
        spatial_preds = []
        for batch in tqdm(batches, disable=False, desc="activations"):
    ...
    def read_statistics(
        self, npz_path: Optional[str], activations: Tuple[np.ndarray, np.ndarray]
...
def _download_inception_model():
    ...
    with requests.get(INCEPTION_V3_URL, stream=True) as r:
        ...
        with open(tmp_path, "wb") as f:
            for chunk in tqdm(r.iter_content(chunk_size=8192), disable=False, desc="downloading inception"):
                f.write(chunk)
